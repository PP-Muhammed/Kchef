...
const Breakfast = () => {
  ...
  useEffect(() => {
    const fetchBreakfastRecipes = async () => {
      try {
        const recipes = await searchRecipes("breakfast");
        setFoods(recipes);
      } catch (error) {
  ...
  const handleFoodClick = async (id) => {
    try {
      ...
      details.videoLink = `https://www.youtube.com/results?search_query=${details.title}+recipe`; // Generate a video search link
